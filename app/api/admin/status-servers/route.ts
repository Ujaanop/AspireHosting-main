...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === 'category') {
      ...
      const newCat = { id: `cat-${Date.now()}`, name: String(name).trim(), description: description ? String(description).trim() : undefined }
    ...
    const newServer = { id: `custom-${Date.now()}`, name: String(name).trim(), url: String(url).trim(), type: type === 'ip' ? 'ip' : 'http', isPreset: false, addedAt: Date.now(), categoryId: categoryId ?? null }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (type === 'category') {
      ...
      data.servers = data.servers.map((s: { categoryId?: string }) => s.categoryId === id ? { ...s, categoryId: null } : s)
      await writeData(data)
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { admins } = body
    if (!Array.isArray(admins)) return NextResponse.json({ error: 'admins must be an array' }, { status: 400 })
    const validAdmins = admins.filter((a: unknown) =>
    ...
    if (validAdmins.length === 0) return NextResponse.json({ error: 'At least one valid admin is required' }, { status: 400 })
