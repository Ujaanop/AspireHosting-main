...
const Navbar: React.FC = () => {
  ...
  const renderDropdown = useCallback((item: NavigationItem) => {
    if (item.dropdownType === 'services' && item.dropdownItems) {
      return (
        <div className="absolute top-full left-0 rounded-b-xl w-[480px] bg-white dark:bg-black/90 backdrop-blur-sm border-t-2 border-top-nav -mt-[2px] shadow-lg opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-200 z-10 p-4">
            ...
            {item.dropdownItems.map((service) => {
              ...
              return (
                <Link
                  key={service.name}
                  href={service.href}
                  className="flex items-start gap-3 p-3 rounded-lg hover:border-secondary border border-transparent hover:hover-gradient transition-colors"
                  prefetch={true}
                >
                  {IconComponent && (
                    <div className="mt-0.5 flex-shrink-0 w-8 h-8 card-primary rounded-lg flex items-center justify-center">
                      <IconComponent className="w-4 h-4 icon-text-primary" />
                    </div>
                  )}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-0.5">{service.name}</h3>
                    {service.description && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 leading-snug">{service.description}</p>
                    )}
                  </div>
                </Link>
              );
    ...
    if (item.dropdownType === 'company' && item.dropdownItems) {
      return (
        <div className="absolute top-full left-0 rounded-b-xl w-[480px] bg-white dark:bg-black/90 backdrop-blur-sm border-t-2 border-top-nav -mt-[2px] shadow-lg opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity duration-200 z-10 p-4">
          <div className="grid grid-cols-2 gap-2">
            {item.dropdownItems.map((dropdownItem) => {
              const ItemIcon = dropdownItem.icon ? getIcon(dropdownItem.icon) : null;
              const isExternal = dropdownItem.href.startsWith('http');
              return (
                <Link
                ...
                >
                  {ItemIcon && (
                    <div className="mt-0.5 flex-shrink-0 w-8 h-8 card-primary rounded-lg flex items-center justify-center">
                      <ItemIcon className="w-4 h-4 icon-text-primary" />
                    </div>
                  )}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-0.5">{dropdownItem.name}</h3>
                    {dropdownItem.description && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 leading-snug">{dropdownItem.description}</p>
                    )}
                  </div>
                </Link>
              );
  ...
  const getTranslatedNavName = useCallback((itemName: string) => {
    ...
    switch (navKey) {
      case 'services': return t('navbar.services') || 'Services';
      ...
      case 'aboutus': return t('navbar.aboutUs');
  ...
  const renderNavigationItem = useCallback((item: NavigationItem) => {
    ...
    const isActive = item.hasDropdown
      ? (
          // Highlight dropdown parents (e.g., Company, Legal) when any child route is active
          Array.isArray((item as any).dropdownItems) && (item as any).dropdownItems.length > 0
            ? (item as any).dropdownItems.some((di: any) => typeof di.href === 'string' && pathname.startsWith(di.href))
    ...
    if (item.hasDropdown) {
      return (
        <div key={item.name} className="relative group">
          <Link href={item.href} className={navLinkClass} prefetch={true}>
    ...
    return (
      <Link key={item.name} href={item.href} className={navLinkClass} prefetch={true}>
        {IconComponent && <IconComponent className="w-4 h-4" />}
        <span>{translatedName}</span>
      </Link>
    );
  ...
  const renderMobileNavigationItem = useCallback((item: NavigationItem) => {
    ...
    if (item.hasDropdown && item.dropdownType === 'services' && item.dropdownItems) {
      const isActive = item.dropdownItems.some(di => pathname.startsWith(di.href));
            ...
            {isDropdownOpen && (
              ...
              >
                <div className="space-y-1">
                  {item.dropdownItems.map((service) => {
                    const ServiceIcon = service.icon ? getIcon(service.icon) : null;
                    const isServiceActive = pathname.startsWith(service.href);
                    return (
                      <Link
                        key={service.name}
                        href={service.href}
                        className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${isServiceActive
                        ...
                        prefetch={true}
                      >
                        {ServiceIcon && <ServiceIcon className="w-4 h-4 flex-shrink-0" />}
                        <div>
    ...
    if (item.hasDropdown && (item.dropdownType === 'company' || item.dropdownType === 'legal') && item.dropdownItems) {
      const isActive = item.dropdownItems.some(di => pathname.startsWith(di.href));
      const isDropdownOpen = mobileDropdownStates[item.name] || false;
            ...
            {isDropdownOpen && (
              ...
              >
                <div className="space-y-1">
                  {item.dropdownItems.map((dropdownItem) => {
                    const ItemIcon = dropdownItem.icon ? getIcon(dropdownItem.icon) : null;
                    const isItemActive = pathname.startsWith(dropdownItem.href);
                    const isExternal = dropdownItem.href.startsWith('http');
                    return (
                      <Link
                        key={dropdownItem.name}
                        href={dropdownItem.href}
                        className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors ${isItemActive
                          ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400'
                          : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white'
                        }`}
                        onClick={closeMobileMenu}
        ...
        <div className="px-4 sm:px-0 max-w-7xl mx-auto">
          <div className="flex items-center">
            ...
            <div className="flex-shrink-0 flex items-center mr-6">
              <Link href="/" className="flex items-center space-x-2 py-2" prefetch={true} onClick={closeMobileMenu}>
            ...
            {/* Right side - Social icons, Language Selector, Theme Toggle and Client Space */}
          ...
          {isMobileMenuOpen && (
            ...
            >
              ...
              >
                ...
                {/* Bottom Section - Theme, Social, and Client Space */}
                <motion.div
