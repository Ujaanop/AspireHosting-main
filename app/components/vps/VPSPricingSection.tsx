...
import { DiscountedPriceHorizontal } from "../ui/DiscountedPrice"

const config = vpsConfig as unknown as VPSConfig
...
export default function VPSPricingSection() {
  ...
  const rawPlans = config.plans[selectedCPU] || config.plans[config.planTypes[0].id]
  // Include plans that are available for the selected location OR explicitly marked coming soon for that location
  const currentPlans = rawPlans.filter((p) => {
    const isAvailableHere = Array.isArray(p.locations) && p.locations.includes(selectedLocation)
    const isComingSoonHere = Boolean(p.locationComingSoon?.[selectedLocation])
    return isAvailableHere || isComingSoonHere
  })
  ...
  // Ensure selected CPU is valid for the current location on mount/location change
  useEffect(() => {
    if (!availableCPUs.includes(selectedCPU) && availableCPUs.length > 0) {
      setSelectedCPU(availableCPUs[0])
      setCurrentPage(1)
    }
  }, [selectedLocation])
      ...
      <div className="relative z-10 mt-16 max-w-7xl mx-auto">
        ...
        >
          {currentPagePlans.map((plan, index) => {
            const isAvailableHere = Array.isArray(plan.locations) && plan.locations.includes(selectedLocation)
            const isComingSoonHere = Boolean(plan.locationComingSoon?.[selectedLocation])
            const isComingSoon = isComingSoonHere || !isAvailableHere
            return (
            <motion.div
              key={plan.id}
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  {isComingSoon ? (
                    <div className="orbitron-font w-full sm:w-auto bg-gray-200 dark:bg-gray-800/40 text-gray-500 dark:text-gray-400 px-6 py-2 rounded-lg font-medium border border-secondary cursor-not-allowed">
                      Coming Soon
                    </div>
                  ) : (
                  <a 
                    href={plan.locationOrderLinks?.[selectedLocation] || plan.orderLink}
